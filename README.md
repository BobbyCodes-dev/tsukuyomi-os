...
This repository hosts public releases only. Source code and build files are not published here.
...
- **AI Agent** — chat with Anthropic, OpenAI-compatible, Gemini, local Ollama, or Ollama Cloud models, with tool calling to open other apps in the OS. Built into the right side of the desktop screen (press Tab to switch focus into the chat), or can be opened as its own window for a bigger view. Provider, model, endpoint, and API key are configured in Settings, where switching providers auto-fills a default endpoint and fetches that provider's available models.
...
1. Download `tsukuyomi.exe` from the [Releases](../../releases) page.
...
Freeware — see [LICENSE](LICENSE). Free to download and use, source stays closed, no redistribution or reverse engineering without permission.
