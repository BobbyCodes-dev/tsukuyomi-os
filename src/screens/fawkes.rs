...
use crate::store::fawkes::{self, FawkesResult, FawkesSession};
...
pub struct FawkesState {
    user_id: i64,
    sessions: Vec<FawkesSession>,
    results: Vec<FawkesResult>,
    selected: usize,
    mode: Mode,
...
impl FawkesState {
    pub fn new(user_id: i64) -> Self {
        ...
        log.push("Fawkes - Facial Recognition Cloaking".to_string());
        log.push("Requires fawkes CLI (pip install fawkes).".to_string());
        log.push("Press 'n' to start a new query.".to_string());
        let mut state = FawkesState {
            user_id,
            sessions: Vec::new(),
            results: Vec::new(),
            selected: 0,
    ...
    fn refresh(&mut self) {
        match fawkes::list_sessions(self.user_id) {
            Ok(sessions) => {
    ...
    pub fn poll_run(&mut self) {
        ...
        match rx.try_recv() {
            Ok((session_id, findings)) => {
                ...
                let _ = fawkes::finish_session(self.user_id, session_id, count, "complete");
                self.log.push(format!("Fawkes cloaking finished: {} images processed.", count));
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
                self.refresh();
            ...
            Err(mpsc::TryRecvError::Disconnected) => {
                self.log.push("Fawkes thread disconnected.".to_string());
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
            }
...
fn start_query(state: &mut FawkesState) {
    let query = state.query.value.trim().to_string();
    if query.is_empty() {
        state.status = "Query is required.".to_string();
        return;
    }
    let session_id = match fawkes::create_session(state.user_id, &query) {
        Ok(id) => id,
        Err(e) => {
    ...
    state.status = "Running Fawkes...".to_string();
    state.log.push(format!("Starting Fawkes: {query}"));
    state.running = true;
...
fn run_tool_pipeline(query: &str) -> Vec<(String, String, String)> {
    ...
    let cli_check = Command::new("fawkes").arg("--version").output();
    ...
    if !installed {
        return vec![(
            ...
            "Fawkes CLI not found. CLI_INSTALL_PLACEHOLDER".to_string(),
            "https://github.com/shawn-shan/fawkes".to_string(),
        )];
    }

    // Run the tool
    let output = Command::new("fawkes")
        .arg("--query")
        .arg(query)
        .arg("--format")
        .arg("json")
    ...
    match output {
        ...
        Ok(out) => {
            ...
            vec![("pipeline_error".to_string(), format!("Fawkes error: {stderr}"), "".to_string())]
        }
        Err(e) => {
            vec![("pipeline_error".to_string(), format!("Failed to execute Fawkes: {e}"), "".to_string())]
        }
    }
}
...
fn parse_tool_output(stdout: &str) -> Vec<(String, String, String)> {
    ...
    if trimmed.is_empty() {
        return vec![("pipeline".to_string(), "No output from Fawkes.".to_string(), "".to_string())];
    }
...
fn load_results(state: &mut FawkesState) {
    if let Some(session) = state.sessions.get(state.selected) {
        match fawkes::list_results(state.user_id, session.id) {
            Ok(results) => {
...
pub fn draw(frame: &mut Frame, area: Rect, state: &FawkesState) {
    let rect = widgets::centered_fixed(100, area.height.min(30), area);
    let block = widgets::form_block("Fawkes - Facial Recognition Cloaking");
...
fn draw_list(frame: &mut Frame, area: Rect, state: &FawkesState) {
...
fn draw_new(frame: &mut Frame, area: Rect, state: &FawkesState) {
    let mut lines = vec![
        Line::styled(format!("New Fawkes Query"), theme::title_style()),
        Line::raw(""),
        field_line("Image directory (e.g. ./photos)", state.query.display(), state.focus == 0),
        Line::raw(""),
        Line::styled("Tab: move  Enter: run  Esc: cancel", theme::hint_style()),
        Line::styled(
            "Cloaks images against facial recognition by adding perturbations.",
            theme::hint_style(),
        ),
    ];
    if !state.status.is_empty() {
...
fn draw_detail(frame: &mut Frame, area: Rect, state: &FawkesState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
...
fn draw_running(frame: &mut Frame, area: Rect, state: &FawkesState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
    ...
    let header = vec![
        Line::styled("Fawkes Running", theme::title_style()),
        Line::raw(""),
        Line::from(vec![
            Span::styled("Query: ", theme::title_style()),
...
fn handle_list_key(state: &mut FawkesState, key: KeyEvent) -> Action {
    match key.code {
        ...
        KeyCode::Char('d') => {
            if let Some(session) = state.sessions.get(state.selected) {
                let id = session.id;
                match fawkes::delete_session(state.user_id, id) {
                    Ok(()) => {
...
fn handle_new_key(state: &mut FawkesState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
...
fn handle_running_key(state: &mut FawkesState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
            state.mode = Mode::List;
...
pub fn handle_key(state: &mut FawkesState, key: KeyEvent) -> Action {
