...
use crate::store::reconftw::{self, ReconFtwResult, ReconFtwSession};
...
pub struct ReconFtwState {
    user_id: i64,
    sessions: Vec<ReconFtwSession>,
    results: Vec<ReconFtwResult>,
    selected: usize,
    mode: Mode,
...
impl ReconFtwState {
    pub fn new(user_id: i64) -> Self {
        ...
        log.push("reconFTW - Full Reconnaissance Framework".to_string());
        log.push("Requires reconftw (git clone + ./install.sh). Many dependencies.".to_string());
        log.push("Press 'n' to start a new query.".to_string());
        let mut state = ReconFtwState {
            user_id,
            sessions: Vec::new(),
            results: Vec::new(),
            selected: 0,
    ...
    fn refresh(&mut self) {
        match reconftw::list_sessions(self.user_id) {
            Ok(sessions) => {
    ...
    pub fn poll_run(&mut self) {
        ...
        match rx.try_recv() {
            Ok((session_id, findings)) => {
                ...
                let _ = reconftw::finish_session(self.user_id, session_id, count, "complete");
                self.log.push(format!("reconFTW scan finished: {} findings.", count));
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
                self.refresh();
            ...
            Err(mpsc::TryRecvError::Disconnected) => {
                self.log.push("ReconFtw thread disconnected.".to_string());
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
            }
...
fn start_query(state: &mut ReconFtwState) {
    let query = state.query.value.trim().to_string();
    if query.is_empty() {
        state.status = "Query is required.".to_string();
        return;
    }
    let session_id = match reconftw::create_session(state.user_id, &query) {
        Ok(id) => id,
        Err(e) => {
    ...
    state.status = "Running ReconFtw...".to_string();
    state.log.push(format!("Starting ReconFtw: {query}"));
    state.running = true;
...
fn run_tool_pipeline(query: &str) -> Vec<(String, String, String)> {
    ...
    let cli_check = Command::new("reconftw").arg("--version").output();
    ...
    if !installed {
        return vec![(
            ...
            "ReconFtw CLI not found. CLI_INSTALL_PLACEHOLDER".to_string(),
            "https://github.com/six2dez/reconftw".to_string(),
        )];
    }

    // Run the tool
    let output = Command::new("reconftw")
        .arg("--query")
        .arg(query)
        .arg("--format")
        .arg("json")
    ...
    match output {
        ...
        Ok(out) => {
            ...
            vec![("pipeline_error".to_string(), format!("ReconFtw error: {stderr}"), "".to_string())]
        }
        Err(e) => {
            vec![("pipeline_error".to_string(), format!("Failed to execute ReconFtw: {e}"), "".to_string())]
        }
    }
}
...
fn parse_tool_output(stdout: &str) -> Vec<(String, String, String)> {
    ...
    if trimmed.is_empty() {
        return vec![("pipeline".to_string(), "No output from ReconFtw.".to_string(), "".to_string())];
    }
...
fn load_results(state: &mut ReconFtwState) {
    if let Some(session) = state.sessions.get(state.selected) {
        match reconftw::list_results(state.user_id, session.id) {
            Ok(results) => {
...
pub fn draw(frame: &mut Frame, area: Rect, state: &ReconFtwState) {
    let rect = widgets::centered_fixed(100, area.height.min(30), area);
    let block = widgets::form_block("reconFTW - Full Reconnaissance Framework");
...
fn draw_list(frame: &mut Frame, area: Rect, state: &ReconFtwState) {
...
fn draw_new(frame: &mut Frame, area: Rect, state: &ReconFtwState) {
    let mut lines = vec![
        Line::styled(format!("New ReconFtw Query"), theme::title_style()),
        Line::raw(""),
        field_line("Target domain (e.g. example.com)", state.query.display(), state.focus == 0),
        Line::raw(""),
        Line::styled("Tab: move  Enter: run  Esc: cancel", theme::hint_style()),
        Line::styled(
            "Runs full recon pipeline: subdomain enumeration, port scan, vulnerability checks.",
            theme::hint_style(),
        ),
    ];
    if !state.status.is_empty() {
...
fn draw_detail(frame: &mut Frame, area: Rect, state: &ReconFtwState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
...
fn draw_running(frame: &mut Frame, area: Rect, state: &ReconFtwState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
    ...
    let header = vec![
        Line::styled("ReconFtw Running", theme::title_style()),
        Line::raw(""),
        Line::from(vec![
            Span::styled("Query: ", theme::title_style()),
...
fn handle_list_key(state: &mut ReconFtwState, key: KeyEvent) -> Action {
    match key.code {
        ...
        KeyCode::Char('d') => {
            if let Some(session) = state.sessions.get(state.selected) {
                let id = session.id;
                match reconftw::delete_session(state.user_id, id) {
                    Ok(()) => {
...
fn handle_new_key(state: &mut ReconFtwState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
...
fn handle_running_key(state: &mut ReconFtwState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
            state.mode = Mode::List;
...
pub fn handle_key(state: &mut ReconFtwState, key: KeyEvent) -> Action {
