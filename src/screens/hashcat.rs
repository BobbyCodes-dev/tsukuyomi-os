...
use crate::store::hashcat::{self, HashcatResult, HashcatSession};
...
pub struct HashcatState {
    user_id: i64,
    sessions: Vec<HashcatSession>,
    results: Vec<HashcatResult>,
    selected: usize,
    mode: Mode,
...
impl HashcatState {
    pub fn new(user_id: i64) -> Self {
        ...
        log.push("Hashcat: GPU-accelerated password recovery — 300+ hash types, attack ".to_string());
        log.push("Requires hashcat (apt install hashcat) and GPU drivers. Enter hash file + mode.".to_string());
        log.push("Press 'n' to start a new session.".to_string());
        let mut state = HashcatState {
            user_id,
            sessions: Vec::new(),
            results: Vec::new(),
            selected: 0,
    ...
    fn refresh(&mut self) {
        match hashcat::list_sessions(self.user_id) {
            Ok(sessions) => {
    ...
    pub fn poll_run(&mut self) {
        ...
        match rx.try_recv() {
            Ok((session_id, findings)) => {
                ...
                let _ = hashcat::finish_session(self.user_id, session_id, count, "complete");
                self.log.push(format!("Hashcat finished: {count} findings.", count = count));
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
                self.refresh();
            ...
            Err(mpsc::TryRecvError::Disconnected) => {
                self.log.push("Hashcat thread disconnected.".to_string());
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
            }
...
fn start_query(state: &mut HashcatState) {
    let query = state.query.value.trim().to_string();
    if query.is_empty() {
        state.status = "Input is required.".to_string();
        return;
    }
    let session_id = match hashcat::create_session(state.user_id, &query) {
        Ok(id) => id,
        Err(e) => {
    ...
    state.status = "Running Hashcat...".to_string();
    state.log.push(format!("Starting Hashcat: {query}", query = query));
    state.running = true;
    ...
    std::thread::spawn(move || {
        let findings = run_hashcat_pipeline(&query);
        let _ = tx.send((session_id, findings));
    });
}

/// Runs the hashcat CLI in a subprocess.
/// Returns a list of (step, finding, source_url) tuples.
/// If hashcat is not installed, returns a single informational finding.
fn run_hashcat_pipeline(query: &str) -> Vec<(String, String, String)> {
    use std::process::Command;

    let check = Command::new("hashcat").arg("--version").output();
    ...
    if !installed {
        return vec![(
            ...
            "hashcat not found. Install with apt install hashcat or pip install hashcat".to_string(),
    ...
    match output {
        Ok(out) if out.status.success() => {
            ...
            parse_hashcat_output(&stdout)
        }
        Ok(out) => {
            let stderr = String::from_utf8_lossy(&out.stderr).to_string();
            vec![("pipeline_error".to_string(), format!("hashcat error: {stderr}", stderr = stderr), "".to_string())]
        }
        Err(e) => {
            vec![("pipeline_error".to_string(), format!("Failed to execute hashcat: {e}", e = e), "".to_string())]
        }
    }
}

fn parse_hashcat_output(stdout: &str) -> Vec<(String, String, String)> {
    let trimmed = stdout.trim();
    if trimmed.is_empty() {
        return vec![("pipeline".to_string(), "No output from hashcat.".to_string(), "".to_string())];
    }
    vec![("result".to_string(), trimmed.to_string(), "".to_string())]
}

fn load_results(state: &mut HashcatState) {
    if let Some(session) = state.sessions.get(state.selected) {
        match hashcat::list_results(state.user_id, session.id) {
            Ok(results) => {
...
pub fn draw(frame: &mut Frame, area: Rect, state: &HashcatState) {
    let rect = widgets::centered_fixed(100, area.height.min(30), area);
    let block = widgets::form_block("Hashcat");
...
fn draw_list(frame: &mut Frame, area: Rect, state: &HashcatState) {
...
fn draw_new(frame: &mut Frame, area: Rect, state: &HashcatState) {
    let mut lines = vec![
        Line::styled("New Hashcat Session", theme::title_style()),
        Line::raw(""),
        field_line("Hash file path", state.query.display(), state.focus == 0),
        Line::raw(""),
        Line::styled("Tab: move  Enter: run  Esc: cancel", theme::hint_style()),
        Line::styled("Requires hashcat (apt install hashcat) and GPU drivers. Enter hash file + mode.", theme::hint_style()),
    ];
    if !state.status.is_empty() {
        lines.push(Line::styled(state.status.clone(), theme::clock_style()));
    }
...
fn draw_detail(frame: &mut Frame, area: Rect, state: &HashcatState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
...
fn draw_running(frame: &mut Frame, area: Rect, state: &HashcatState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
    ...
    let header = vec![
        Line::styled("Hashcat Running", theme::title_style()),
        Line::raw(""),
        Line::from(vec![
            Span::styled("Input: ", theme::title_style()),
...
fn handle_list_key(state: &mut HashcatState, key: KeyEvent) -> Action {
    match key.code {
        ...
        KeyCode::Char('d') => {
            if let Some(session) = state.sessions.get(state.selected) {
                let id = session.id;
                match hashcat::delete_session(state.user_id, id) {
                    Ok(()) => {
...
fn handle_new_key(state: &mut HashcatState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
...
fn handle_running_key(state: &mut HashcatState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
            state.mode = Mode::List;
...
pub fn handle_key(state: &mut HashcatState, key: KeyEvent) -> Action {
