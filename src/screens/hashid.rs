...
use crate::store::hashid::{self, HashidResult, HashidSession};
...
pub struct HashidState {
    user_id: i64,
    sessions: Vec<HashidSession>,
    results: Vec<HashidResult>,
    selected: usize,
    mode: Mode,
...
impl HashidState {
    pub fn new(user_id: i64) -> Self {
        ...
        log.push("Hashid: Identify hash types from a hash string — supports 200+ hash ".to_string());
        log.push("Requires hashid (pip install hashid). Paste a hash to identify its type.".to_string());
        log.push("Press 'n' to start a new session.".to_string());
        let mut state = HashidState {
            user_id,
            sessions: Vec::new(),
            results: Vec::new(),
            selected: 0,
    ...
    fn refresh(&mut self) {
        match hashid::list_sessions(self.user_id) {
            Ok(sessions) => {
    ...
    pub fn poll_run(&mut self) {
        ...
        match rx.try_recv() {
            Ok((session_id, findings)) => {
                ...
                let _ = hashid::finish_session(self.user_id, session_id, count, "complete");
                self.log.push(format!("Hashid finished: {count} findings.", count = count));
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
                self.refresh();
            ...
            Err(mpsc::TryRecvError::Disconnected) => {
                self.log.push("Hashid thread disconnected.".to_string());
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
            }
...
fn start_query(state: &mut HashidState) {
    let query = state.query.value.trim().to_string();
    if query.is_empty() {
        state.status = "Input is required.".to_string();
        return;
    }
    let session_id = match hashid::create_session(state.user_id, &query) {
        Ok(id) => id,
        Err(e) => {
    ...
    state.status = "Running Hashid...".to_string();
    state.log.push(format!("Starting Hashid: {query}", query = query));
    state.running = true;
    ...
    std::thread::spawn(move || {
        let findings = run_hashid_pipeline(&query);
        let _ = tx.send((session_id, findings));
    });
}

/// Runs the hashid CLI in a subprocess.
/// Returns a list of (step, finding, source_url) tuples.
/// If hashid is not installed, returns a single informational finding.
fn run_hashid_pipeline(query: &str) -> Vec<(String, String, String)> {
    use std::process::Command;

    let check = Command::new("hashid").arg("--version").output();
    ...
    if !installed {
        return vec![(
            ...
            "hashid not found. Install with apt install hashid or pip install hashid".to_string(),
            "https://github.com/openwall/john".to_string(),
        )];
    }

    let output = Command::new("hashid")
        .arg(query)
        .output();
    ...
    match output {
        Ok(out) if out.status.success() => {
            ...
            parse_hashid_output(&stdout)
        }
        Ok(out) => {
            let stderr = String::from_utf8_lossy(&out.stderr).to_string();
            vec![("pipeline_error".to_string(), format!("hashid error: {stderr}", stderr = stderr), "".to_string())]
        }
        Err(e) => {
            vec![("pipeline_error".to_string(), format!("Failed to execute hashid: {e}", e = e), "".to_string())]
        }
    }
}

fn parse_hashid_output(stdout: &str) -> Vec<(String, String, String)> {
    let trimmed = stdout.trim();
    if trimmed.is_empty() {
        return vec![("pipeline".to_string(), "No output from hashid.".to_string(), "".to_string())];
    }
    vec![("result".to_string(), trimmed.to_string(), "".to_string())]
}

fn load_results(state: &mut HashidState) {
    if let Some(session) = state.sessions.get(state.selected) {
        match hashid::list_results(state.user_id, session.id) {
            Ok(results) => {
...
pub fn draw(frame: &mut Frame, area: Rect, state: &HashidState) {
    let rect = widgets::centered_fixed(100, area.height.min(30), area);
    let block = widgets::form_block("Hashid");
...
fn draw_list(frame: &mut Frame, area: Rect, state: &HashidState) {
...
fn draw_new(frame: &mut Frame, area: Rect, state: &HashidState) {
    let mut lines = vec![
        Line::styled("New Hashid Session", theme::title_style()),
        Line::raw(""),
        field_line("Hash string to identify", state.query.display(), state.focus == 0),
        Line::raw(""),
        Line::styled("Tab: move  Enter: run  Esc: cancel", theme::hint_style()),
        Line::styled("Requires hashid (pip install hashid). Paste a hash to identify its type.", theme::hint_style()),
    ];
    if !state.status.is_empty() {
        lines.push(Line::styled(state.status.clone(), theme::clock_style()));
    }
...
fn draw_detail(frame: &mut Frame, area: Rect, state: &HashidState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
...
fn draw_running(frame: &mut Frame, area: Rect, state: &HashidState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
    ...
    let header = vec![
        Line::styled("Hashid Running", theme::title_style()),
        Line::raw(""),
        Line::from(vec![
            Span::styled("Input: ", theme::title_style()),
...
fn handle_list_key(state: &mut HashidState, key: KeyEvent) -> Action {
    match key.code {
        ...
        KeyCode::Char('d') => {
            if let Some(session) = state.sessions.get(state.selected) {
                let id = session.id;
                match hashid::delete_session(state.user_id, id) {
                    Ok(()) => {
...
fn handle_new_key(state: &mut HashidState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
...
fn handle_running_key(state: &mut HashidState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
            state.mode = Mode::List;
...
pub fn handle_key(state: &mut HashidState, key: KeyEvent) -> Action {
