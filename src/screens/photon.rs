...
use crate::store::photon::{self, PhotonResult, PhotonSession};
...
pub struct PhotonState {
    user_id: i64,
    sessions: Vec<PhotonSession>,
    results: Vec<PhotonResult>,
    selected: usize,
    mode: Mode,
...
impl PhotonState {
    pub fn new(user_id: i64) -> Self {
        ...
        log.push("Photon - Web Crawler OSINT".to_string());
        log.push("Requires photon (pip install photon).".to_string());
    ...
    fn refresh(&mut self) {
        match photon::list_sessions(self.user_id) {
            Ok(sessions) => {
    ...
    pub fn poll_run(&mut self) {
        ...
        match rx.try_recv() {
            Ok((session_id, findings)) => {
                ...
                let _ = photon::finish_session(self.user_id, session_id, count, "complete");
                self.log.push(format!("Photon crawl finished: {} results extracted.", count));
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
                self.refresh();
            ...
            Err(mpsc::TryRecvError::Disconnected) => {
                self.log.push("Photon thread disconnected.".to_string());
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
            }
...
fn start_query(state: &mut PhotonState) {
    let query = state.query.value.trim().to_string();
    if query.is_empty() {
        state.status = "Query is required.".to_string();
        return;
    }
    let session_id = match photon::create_session(state.user_id, &query) {
        Ok(id) => id,
        Err(e) => {
    ...
    state.status = "Running Photon...".to_string();
    state.log.push(format!("Starting Photon: {query}"));
    state.running = true;
...
fn run_tool_pipeline(query: &str) -> Vec<(String, String, String)> {
    ...
    let cli_check = Command::new("photon").arg("--version").output();
    ...
    if !installed {
        return vec![(
            ...
            "Photon CLI not found. CLI_INSTALL_PLACEHOLDER".to_string(),
            "https://github.com/s0md3v/Photon".to_string(),
        )];
    }

    // Run the tool
    let output = Command::new("photon")
        .arg("--query")
        .arg(query)
        .arg("--format")
        .arg("json")
    ...
    match output {
        ...
        Ok(out) => {
            ...
            vec![("pipeline_error".to_string(), format!("Photon error: {stderr}"), "".to_string())]
        }
        Err(e) => {
            vec![("pipeline_error".to_string(), format!("Failed to execute Photon: {e}"), "".to_string())]
        }
    }
}
...
fn parse_tool_output(stdout: &str) -> Vec<(String, String, String)> {
    ...
    if trimmed.is_empty() {
        return vec![("pipeline".to_string(), "No output from Photon.".to_string(), "".to_string())];
    }
...
fn load_results(state: &mut PhotonState) {
    if let Some(session) = state.sessions.get(state.selected) {
        match photon::list_results(state.user_id, session.id) {
            Ok(results) => {
...
pub fn draw(frame: &mut Frame, area: Rect, state: &PhotonState) {
    let rect = widgets::centered_fixed(100, area.height.min(30), area);
    let block = widgets::form_block("Photon - Web Crawler OSINT");
...
fn draw_list(frame: &mut Frame, area: Rect, state: &PhotonState) {
...
fn draw_new(frame: &mut Frame, area: Rect, state: &PhotonState) {
    let mut lines = vec![
        Line::styled(format!("New Photon Query"), theme::title_style()),
        Line::raw(""),
        field_line("Target URL (e.g. https://example.com)", state.query.display(), state.focus == 0),
        Line::raw(""),
        Line::styled("Tab: move  Enter: run  Esc: cancel", theme::hint_style()),
        Line::styled(
            "Crawls target sites for URLs, emails, social media, files, and intelligence.",
            theme::hint_style(),
        ),
    ];
    if !state.status.is_empty() {
...
fn draw_detail(frame: &mut Frame, area: Rect, state: &PhotonState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
...
fn draw_running(frame: &mut Frame, area: Rect, state: &PhotonState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
    ...
    let header = vec![
        Line::styled("Photon Running", theme::title_style()),
        Line::raw(""),
        Line::from(vec![
            Span::styled("Query: ", theme::title_style()),
...
fn handle_list_key(state: &mut PhotonState, key: KeyEvent) -> Action {
    match key.code {
        ...
        KeyCode::Char('d') => {
            if let Some(session) = state.sessions.get(state.selected) {
                let id = session.id;
                match photon::delete_session(state.user_id, id) {
                    Ok(()) => {
...
fn handle_new_key(state: &mut PhotonState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
...
fn handle_running_key(state: &mut PhotonState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
            state.mode = Mode::List;
...
pub fn handle_key(state: &mut PhotonState, key: KeyEvent) -> Action {
