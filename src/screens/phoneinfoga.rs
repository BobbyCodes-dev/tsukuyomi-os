...
use crate::store::phoneinfoga::{self, PhoneInfogaResult, PhoneInfogaSession};
...
pub struct PhoneInfogaState {
    user_id: i64,
    sessions: Vec<PhoneInfogaSession>,
    results: Vec<PhoneInfogaResult>,
    selected: usize,
    mode: Mode,
...
impl PhoneInfogaState {
    pub fn new(user_id: i64) -> Self {
        let mut log = widgets::LogPanel::new(200);
        log.push("PhoneInfoga - Phone Number OSINT".to_string());
        log.push("Requires phoneinfoga CLI (go install).".to_string());
        log.push("Press 'n' to start a new query.".to_string());
        let mut state = PhoneInfogaState {
            user_id,
            sessions: Vec::new(),
            results: Vec::new(),
            selected: 0,
    ...
    fn refresh(&mut self) {
        match phoneinfoga::list_sessions(self.user_id) {
            Ok(sessions) => {
    ...
    pub fn poll_run(&mut self) {
        ...
        match rx.try_recv() {
            Ok((session_id, findings)) => {
                ...
                let _ = phoneinfoga::finish_session(self.user_id, session_id, count, "complete");
                self.log.push(format!("PhoneInfoga scan finished: {} findings.", count));
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
                self.refresh();
            ...
            Err(mpsc::TryRecvError::Disconnected) => {
                self.log.push("PhoneInfoga thread disconnected.".to_string());
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
            }
...
fn start_query(state: &mut PhoneInfogaState) {
    let query = state.query.value.trim().to_string();
    if query.is_empty() {
        state.status = "Query is required.".to_string();
        return;
    }
    let session_id = match phoneinfoga::create_session(state.user_id, &query) {
        Ok(id) => id,
        Err(e) => {
    ...
    state.status = "Running PhoneInfoga...".to_string();
    state.log.push(format!("Starting PhoneInfoga: {query}"));
    state.running = true;
...
fn run_tool_pipeline(query: &str) -> Vec<(String, String, String)> {
    ...
    let cli_check = Command::new("phoneinfoga").arg("--version").output();
    ...
    if !installed {
        return vec![(
            ...
            "PhoneInfoga CLI not found. CLI_INSTALL_PLACEHOLDER".to_string(),
            "https://github.com/sundowndev/phoneinfoga".to_string(),
        )];
    }

    // Run the tool
    let output = Command::new("phoneinfoga")
        .arg("--query")
        .arg(query)
        .arg("--format")
        .arg("json")
    ...
    match output {
        ...
        Ok(out) => {
            ...
            vec![("pipeline_error".to_string(), format!("PhoneInfoga error: {stderr}"), "".to_string())]
        }
        Err(e) => {
            vec![("pipeline_error".to_string(), format!("Failed to execute PhoneInfoga: {e}"), "".to_string())]
        }
    }
}
...
fn parse_tool_output(stdout: &str) -> Vec<(String, String, String)> {
    ...
    if trimmed.is_empty() {
        return vec![("pipeline".to_string(), "No output from PhoneInfoga.".to_string(), "".to_string())];
    }
...
fn load_results(state: &mut PhoneInfogaState) {
    if let Some(session) = state.sessions.get(state.selected) {
        match phoneinfoga::list_results(state.user_id, session.id) {
            Ok(results) => {
...
pub fn draw(frame: &mut Frame, area: Rect, state: &PhoneInfogaState) {
    let rect = widgets::centered_fixed(100, area.height.min(30), area);
    let block = widgets::form_block("PhoneInfoga - Phone Number OSINT");
...
fn draw_list(frame: &mut Frame, area: Rect, state: &PhoneInfogaState) {
...
fn draw_new(frame: &mut Frame, area: Rect, state: &PhoneInfogaState) {
    let mut lines = vec![
        Line::styled(format!("New PhoneInfoga Query"), theme::title_style()),
        Line::raw(""),
        field_line("Phone number (e.g. +123****7890)", state.query.display(), state.focus == 0),
        Line::raw(""),
        Line::styled("Tab: move  Enter: run  Esc: cancel", theme::hint_style()),
        Line::styled(
            "Scans phone numbers for carrier, line type, reputation, and OSINT footprinting.",
            theme::hint_style(),
        ),
    ];
    if !state.status.is_empty() {
...
fn draw_detail(frame: &mut Frame, area: Rect, state: &PhoneInfogaState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
...
fn draw_running(frame: &mut Frame, area: Rect, state: &PhoneInfogaState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
    ...
    let header = vec![
        Line::styled("PhoneInfoga Running", theme::title_style()),
        Line::raw(""),
        Line::from(vec![
            Span::styled("Query: ", theme::title_style()),
...
fn handle_list_key(state: &mut PhoneInfogaState, key: KeyEvent) -> Action {
    match key.code {
        ...
        KeyCode::Char('d') => {
            if let Some(session) = state.sessions.get(state.selected) {
                let id = session.id;
                match phoneinfoga::delete_session(state.user_id, id) {
                    Ok(()) => {
...
fn handle_new_key(state: &mut PhoneInfogaState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
...
fn handle_running_key(state: &mut PhoneInfogaState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
            state.mode = Mode::List;
...
pub fn handle_key(state: &mut PhoneInfogaState, key: KeyEvent) -> Action {
