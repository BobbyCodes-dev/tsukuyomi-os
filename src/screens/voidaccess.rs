...
use crate::store::voidaccess::{self, VoidAccessResult, VoidAccessSession};
...
pub struct VoidAccessState {
    user_id: i64,
    sessions: Vec<VoidAccessSession>,
    results: Vec<VoidAccessResult>,
    selected: usize,
    mode: Mode,
...
impl VoidAccessState {
    pub fn new(user_id: i64) -> Self {
        let mut log = widgets::LogPanel::new(200);
        log.push("VoidAccess: Dark web OSINT platform.".to_string());
        log.push("Requires local Tor and voidaccess CLI (pip install voidaccess).".to_string());
        log.push("Press 'n' to start a new query.".to_string());
        let mut state = VoidAccessState {
            user_id,
            sessions: Vec::new(),
            results: Vec::new(),
            selected: 0,
    ...
    fn refresh(&mut self) {
        match voidaccess::list_sessions(self.user_id) {
            Ok(sessions) => {
    ...
    pub fn poll_run(&mut self) {
        ...
        match rx.try_recv() {
            Ok((session_id, findings)) => {
                ...
                let _ = voidaccess::finish_session(self.user_id, session_id, count, "complete");
                self.log.push(format!("VoidAccess query finished: {count} findings."));
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
                self.refresh();
            ...
            Err(mpsc::TryRecvError::Disconnected) => {
                self.log.push("VoidAccess pipeline thread disconnected.".to_string());
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
            }
...
fn start_query(state: &mut VoidAccessState) {
    let query = state.query.value.trim().to_string();
    if query.is_empty() {
        state.status = "Query is required.".to_string();
        return;
    }
    let session_id = match voidaccess::create_session(state.user_id, &query) {
        Ok(id) => id,
        Err(e) => {
    ...
    state.status = "Running VoidAccess pipeline...".to_string();
    state.log.push(format!("Starting VoidAccess query: {query}"));
    state.running = true;
    ...
    std::thread::spawn(move || {
        let findings = run_voidaccess_pipeline(&query);
        let _ = tx.send((session_id, findings));
    });
}

/// Runs the voidaccess CLI pipeline in a subprocess.
/// Returns a list of (step, finding, source_url) tuples.
/// If voidaccess is not installed, returns a single informational finding.
fn run_voidaccess_pipeline(query: &str) -> Vec<(String, String, String)> {
    use std::process::Command;

    // Check if voidaccess is installed
    let va_check = Command::new("voidaccess").arg("--version").output();
    let installed = match va_check {
    ...
    if !installed {
        return vec![(
            ...
            "voidaccess CLI not found. Install with: pip install voidaccess".to_string(),
            "https://github.com/KatrielMoses/voidaccess".to_string(),
        )];
    }

    // Run the voidaccess pipeline
    let output = Command::new("voidaccess")
        .args(["--query", query, "--format", "json", "--no-headless"])
        .output();

    match output {
        Ok(out) if out.status.success() => {
            let stdout = String::from_utf8_lossy(&out.stdout).to_string();
            parse_voidaccess_output(&stdout)
        }
        Ok(out) => {
            let stderr = String::from_utf8_lossy(&out.stderr).to_string();
            vec![("pipeline_error".to_string(), format!("voidaccess error: {stderr}"), "".to_string())]
        }
        Err(e) => {
            vec![("pipeline_error".to_string(), format!("Failed to execute voidaccess: {e}"), "".to_string())]
        }
    }
}

/// Parse voidaccess JSON output into findings.
/// Expects an array of objects with step, finding, and optional url fields.
fn parse_voidaccess_output(stdout: &str) -> Vec<(String, String, String)> {
    let trimmed = stdout.trim();
    if trimmed.is_empty() {
        return vec![("pipeline".to_string(), "No output from voidaccess.".to_string(), "".to_string())];
    }
...
fn load_results(state: &mut VoidAccessState) {
    if let Some(session) = state.sessions.get(state.selected) {
        match voidaccess::list_results(state.user_id, session.id) {
            Ok(results) => {
...
pub fn draw(frame: &mut Frame, area: Rect, state: &VoidAccessState) {
    let rect = widgets::centered_fixed(100, area.height.min(30), area);
    let block = widgets::form_block("VoidAccess - Dark Web OSINT");
...
fn draw_list(frame: &mut Frame, area: Rect, state: &VoidAccessState) {
...
fn draw_new(frame: &mut Frame, area: Rect, state: &VoidAccessState) {
    let mut lines = vec![
        Line::styled("New VoidAccess Query", theme::title_style()),
        Line::raw(""),
        field_line("Query (email, username, or keyword)", state.query.display(), state.focus == 0),
        Line::raw(""),
        Line::styled("Tab: move  Enter: run  Esc: cancel", theme::hint_style()),
        Line::styled(
            "Runs the VoidAccess 13-step dark web OSINT pipeline locally (requires Tor).",
            theme::hint_style(),
        ),
    ];
    if !state.status.is_empty() {
...
fn draw_detail(frame: &mut Frame, area: Rect, state: &VoidAccessState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
...
fn draw_running(frame: &mut Frame, area: Rect, state: &VoidAccessState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
    ...
    let header = vec![
        Line::styled("VoidAccess Pipeline Running", theme::title_style()),
        Line::raw(""),
        Line::from(vec![
            Span::styled("Query: ", theme::title_style()),
...
fn handle_list_key(state: &mut VoidAccessState, key: KeyEvent) -> Action {
    match key.code {
        ...
        KeyCode::Char('d') => {
            if let Some(session) = state.sessions.get(state.selected) {
                let id = session.id;
                match voidaccess::delete_session(state.user_id, id) {
                    Ok(()) => {
...
fn handle_new_key(state: &mut VoidAccessState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
...
fn handle_running_key(state: &mut VoidAccessState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
            // Return to list — the pipeline thread will still finish and save results
            state.mode = Mode::List;
...
pub fn handle_key(state: &mut VoidAccessState, key: KeyEvent) -> Action {
