...
use crate::store::crunch::{self, CrunchResult, CrunchSession};
...
pub struct CrunchState {
    user_id: i64,
    sessions: Vec<CrunchSession>,
    results: Vec<CrunchResult>,
    selected: usize,
    mode: Mode,
...
impl CrunchState {
    pub fn new(user_id: i64) -> Self {
        ...
        log.push("Crunch: Generate custom wordlists by charset + length — pipe to john".to_string());
        log.push("Requires crunch (apt install crunch). Enter min/max length and charset.".to_string());
        log.push("Press 'n' to start a new session.".to_string());
        let mut state = CrunchState {
            user_id,
            sessions: Vec::new(),
            results: Vec::new(),
            selected: 0,
    ...
    fn refresh(&mut self) {
        match crunch::list_sessions(self.user_id) {
            Ok(sessions) => {
    ...
    pub fn poll_run(&mut self) {
        ...
        match rx.try_recv() {
            Ok((session_id, findings)) => {
                ...
                let _ = crunch::finish_session(self.user_id, session_id, count, "complete");
                self.log.push(format!("Crunch finished: {count} findings.", count = count));
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
                self.refresh();
            ...
            Err(mpsc::TryRecvError::Disconnected) => {
                self.log.push("Crunch thread disconnected.".to_string());
                self.running = false;
                self.run_rx = None;
                self.mode = Mode::List;
            }
...
fn start_query(state: &mut CrunchState) {
    let query = state.query.value.trim().to_string();
    if query.is_empty() {
        state.status = "Input is required.".to_string();
        return;
    }
    let session_id = match crunch::create_session(state.user_id, &query) {
        Ok(id) => id,
        Err(e) => {
    ...
    state.status = "Running Crunch...".to_string();
    state.log.push(format!("Starting Crunch: {query}", query = query));
    state.running = true;
    ...
    std::thread::spawn(move || {
        let findings = run_crunch_pipeline(&query);
        let _ = tx.send((session_id, findings));
    });
}

/// Runs the crunch CLI in a subprocess.
/// Returns a list of (step, finding, source_url) tuples.
/// If crunch is not installed, returns a single informational finding.
fn run_crunch_pipeline(query: &str) -> Vec<(String, String, String)> {
    use std::process::Command;

    let check = Command::new("crunch").arg("--version").output();
    ...
    if !installed {
        return vec![(
            ...
            "crunch not found. Install with apt install crunch or pip install crunch".to_string(),
            "https://github.com/openwall/john".to_string(),
        )];
    }

    let output = Command::new("crunch")
        .args(["8", "8"])
        .arg(query)
        .output();
    ...
    match output {
        Ok(out) if out.status.success() => {
            ...
            parse_crunch_output(&stdout)
        }
        Ok(out) => {
            let stderr = String::from_utf8_lossy(&out.stderr).to_string();
            vec![("pipeline_error".to_string(), format!("crunch error: {stderr}", stderr = stderr), "".to_string())]
        }
        Err(e) => {
            vec![("pipeline_error".to_string(), format!("Failed to execute crunch: {e}", e = e), "".to_string())]
        }
    }
}

fn parse_crunch_output(stdout: &str) -> Vec<(String, String, String)> {
    let trimmed = stdout.trim();
    if trimmed.is_empty() {
        return vec![("pipeline".to_string(), "No output from crunch.".to_string(), "".to_string())];
    }
    vec![("result".to_string(), trimmed.to_string(), "".to_string())]
}

fn load_results(state: &mut CrunchState) {
    if let Some(session) = state.sessions.get(state.selected) {
        match crunch::list_results(state.user_id, session.id) {
            Ok(results) => {
...
pub fn draw(frame: &mut Frame, area: Rect, state: &CrunchState) {
    let rect = widgets::centered_fixed(100, area.height.min(30), area);
    let block = widgets::form_block("Crunch");
...
fn draw_list(frame: &mut Frame, area: Rect, state: &CrunchState) {
...
fn draw_new(frame: &mut Frame, area: Rect, state: &CrunchState) {
    let mut lines = vec![
        Line::styled("New Crunch Session", theme::title_style()),
        Line::raw(""),
        field_line("min_len max_len charset", state.query.display(), state.focus == 0),
        Line::raw(""),
        Line::styled("Tab: move  Enter: run  Esc: cancel", theme::hint_style()),
        Line::styled("Requires crunch (apt install crunch). Enter min/max length and charset.", theme::hint_style()),
    ];
    if !state.status.is_empty() {
        lines.push(Line::styled(state.status.clone(), theme::clock_style()));
    }
...
fn draw_detail(frame: &mut Frame, area: Rect, state: &CrunchState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
...
fn draw_running(frame: &mut Frame, area: Rect, state: &CrunchState) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
    ...
    let header = vec![
        Line::styled("Crunch Running", theme::title_style()),
        Line::raw(""),
        Line::from(vec![
            Span::styled("Input: ", theme::title_style()),
...
fn handle_list_key(state: &mut CrunchState, key: KeyEvent) -> Action {
    match key.code {
        ...
        KeyCode::Char('d') => {
            if let Some(session) = state.sessions.get(state.selected) {
                let id = session.id;
                match crunch::delete_session(state.user_id, id) {
                    Ok(()) => {
...
fn handle_new_key(state: &mut CrunchState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
...
fn handle_running_key(state: &mut CrunchState, key: KeyEvent) -> Action {
    match key.code {
        KeyCode::Esc => {
            state.mode = Mode::List;
...
pub fn handle_key(state: &mut CrunchState, key: KeyEvent) -> Action {
